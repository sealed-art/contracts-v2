...
describe("SealedEditions", function () {
    async function deployExchangeFixture() {
        ...
        return { editions, artist, manifoldContract, sequencer, seller, buyer, owner, delegate, sign, defaultParams:{
            nftContract, uri, cost, startDate, endDate, maxToMint, maxPerWallet, merkleRoot
        } };
    }
    async function getSigs(seller: any, buyer: any, sequencer: any, sign: any, editions: any, nftContract: string,
        ...
        const attestation = await sign(sequencer, {
            ...
            offerHash: ethers.keccak256(new ethers.AbiCoder().encode(
                ["address", "address", "address", "string", "uint256", "uint256", "uint256", "uint256", "uint256", "bytes32", "uint256", "uint256", "uint256"],
                [buyer.address, seller.address, nftContract, uri, cost, startDate, endDate,
                    maxToMint, maxPerWallet, merkleRoot,
                    deadline,
                    counter,
                    nonce,])),
        }, await editions.getAddress())
    ...
    it("basic flow", async function () {
        const { sequencer, seller, buyer, owner, sign, editions, manifoldContract, defaultParams:{ nftContract, uri, cost, startDate, endDate, maxToMint, maxPerWallet, merkleRoot } } = await loadFixture(deployExchangeFixture);
        ...
        const nftId = 4n
        await expect(editions.connect(buyer).mint(1, nftContract, nftId, cost, startDate, endDate, maxToMint, maxPerWallet, seller.address, merkleRoot, {
            value: eth(0.1)
        ...
        await editions.connect(buyer).mint(1, nftContract, nftId, cost, startDate, endDate, maxToMint, maxPerWallet, seller.address, merkleRoot, {
            value: eth(0.1)
        })
        ...
        await expect(editions.connect(buyer).mint(95, nftContract, nftId, cost, startDate, endDate, maxToMint, maxPerWallet, seller.address, merkleRoot, { value: eth(9.4) }))
    ...
    it("editMint", async function () {
        const { sequencer, seller, buyer, sign, editions, defaultParams:{ nftContract, uri, cost, startDate, endDate, maxToMint, maxPerWallet, merkleRoot } } = await loadFixture(deployExchangeFixture);
        ...
        const sigs2 = await getSigs(seller, buyer, sequencer, sign, editions, nftContract, uri, eth(1), 1, endDate+1, 10, maxPerWallet, merkleRoot)
    ...
    it("merkle mint", async function () {
        const { sequencer, seller, buyer, sign, editions, delegate, defaultParams:{ nftContract, uri, startDate, endDate, maxToMint, maxPerWallet } } = await loadFixture(deployExchangeFixture);
        ...
        await editions.connect(buyer).mintWithMerkle(1, nftContract, 4n, cost, startDate, endDate, maxToMint, maxPerWallet, seller.address, merkleRoot,
